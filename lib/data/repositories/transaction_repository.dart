...
class TransactionRepository {
  ...
  // Update transaction
  Future<TransactionModel> updateTransaction(TransactionModel updatedTransaction) async {
  ...
  double calculateClientBalance(String clientId, {String? currencyCode}) {
    ...
    if (currencyCode != null) {
      transactions.removeWhere((transaction) => transaction.currency != currencyCode);
    }
    
    double balance = 0.0;
    
    for (final transaction in transactions) {
      if (transaction.type == AppConstants.transactionTypeIncoming) {
        balance += transaction.amount;
      } else if (transaction.type == AppConstants.transactionTypeOutgoing) {
        balance -= transaction.amount;
      }
    }
  ...
  Map<String, dynamic> generateMonthlyReport(int year, int month, {String? clientId, String? currencyCode}) {
    ...
    if (clientId != null) {
      transactions = getTransactionsByClientAndDateRange(clientId, startDate, endDate);
    } else {
      transactions = getTransactionsByDateRange(startDate, endDate);
    }
    
    if (currencyCode != null) {
      transactions = transactions.where((t) => t.currency == currencyCode).toList();
    }
